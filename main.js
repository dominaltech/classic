// CLASSIC COLLECTION SOLAPUR STOREFRONT CORE JAVASCRIPT SYSTEM
...
(function() {
  ...
  const cachedSettings = localStorage.getItem('classic_store_settings');
  if (cachedSettings) {
    try {
  ...
  async function fetchStoreSettings() {
    ...
    try {
      ...
      if (data) {
        ...
        localStorage.setItem('classic_store_settings', JSON.stringify(data));
      }
    } catch(e) {
  ...
  window.UR_CART = {
    get: function() {
      try {
        return JSON.parse(localStorage.getItem('classic_cart')) || [];
      } catch (e) {
        return [];
      }
    },
    set: function(items) {
      localStorage.setItem('classic_cart', JSON.stringify(items));
      this.updateBadge();
    },
    add: function(item) {
      let cart = this.get();
